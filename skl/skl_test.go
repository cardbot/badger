...
func TestBasic(t *testing.T) {
	...
	l.Put([]byte("key1"), y.ValueStruct{val1, 55})
	l.Put([]byte("key3"), y.ValueStruct{val3, 56})
	l.Put([]byte("key2"), y.ValueStruct{val2, 57})
	...
	l.Put([]byte("key2"), y.ValueStruct{val4, 12})
	v = l.Get([]byte("key2"))
	require.True(t, v.Value != nil)
...
func TestOneKey(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			defer wg.Done()
			l.Put(key, y.ValueStruct{newValue(i), 0})
		}(i)
	}
...
func TestFindNear(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		l.Put([]byte(key), y.ValueStruct{newValue(i), 0})
	}

	n, eq := l.findNear([]byte("00001"), false, false)
	require.NotNil(t, n)
	require.EqualValues(t, "00005", string(n.getKey(l.arena)))
	require.False(t, eq)
	...
	require.NotNil(t, n)
	require.EqualValues(t, "00015", string(n.getKey(l.arena)))
	require.False(t, eq)
	...
	require.NotNil(t, n)
	require.EqualValues(t, "05565", string(n.getKey(l.arena)))
	require.False(t, eq)
	n, eq = l.findNear([]byte("05555"), false, true)
	require.NotNil(t, n)
	require.EqualValues(t, "05555", string(n.getKey(l.arena)))
	require.True(t, eq)
	n, eq = l.findNear([]byte("05555"), true, false)
	require.NotNil(t, n)
	require.EqualValues(t, "05545", string(n.getKey(l.arena)))
	require.False(t, eq)
	...
	require.NotNil(t, n)
	require.EqualValues(t, "09995", string(n.getKey(l.arena)))
	require.True(t, eq)
	n, eq = l.findNear([]byte("09995"), true, false)
	require.NotNil(t, n)
	require.EqualValues(t, "09985", string(n.getKey(l.arena)))
	require.False(t, eq)
...
func TestIteratorSeek(t *testing.T) {
	...
	for i := n - 1; i >= 0; i-- {
		v := i*10 + 1000
		l.Put([]byte(fmt.Sprintf("%05d", i*10+1000)), y.ValueStruct{newValue(v), 0})
	}
	it.Seek([]byte(""))
	require.True(t, it.Valid())
	v := it.Value()
...
func BenchmarkReadWrite(b *testing.B) {
	...
	for i := 0; i <= 10; i++ {
		...
		b.Run(fmt.Sprintf("frac_%d", i), func(b *testing.B) {
			l := NewSkiplist(arenaSize * 32) // TODO: Fix this. Allow arena size to vary with b.N.
			defer l.DecrRef()
			b.ResetTimer()
			var count int
			b.RunParallel(func(pb *testing.PB) {
				...
				for pb.Next() {
					if rng.Float32() < readFrac {
						...
						l.Put(randomKey(rng), y.ValueStruct{value, 0})
					}
				}
			})
		})
